...
- `stl2emp( src: stream [, dst: stream, options: object] ) : stream` ... Parses _STL_ document and generates corresponding _Empower JSON_ fragment
...
```js
    ...
    var json = streams.stream('wd:/output/hello.json');
    // convert STL to Empower JSON
    empower.stl2emp(stl, json);
