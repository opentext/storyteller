...
language (strict subsed of Javascript) designed to allow programs written in C/C++ to be run
...
new web standard created by multi-ventod cooperation (Google, Mozilla, Microsoft, Apple, ...).
...
compiler infrastructure and toolchain library for /WebAssembly/. More details about generating /WebAssembly/
from /Emscripten/ are described [here](https://github.com/kripken/emscripten/wiki/WebAssembly).
...
First tried strategy is to build development version of the clang compiler.
...
This [patch]() fixes all the build errors. 
